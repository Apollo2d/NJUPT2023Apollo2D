...
class DefaultRNG : public boost::mt19937 {
   public:
    static DefaultRNG& instance() {
    ...
    template<class Generator> static DefaultRNG& instance(Generator& gen) {
        instance().seed(gen);
        return instance();
    }

   private:
    DefaultRNG() : boost::mt19937() {}
};
...
// private:
//     UniformRNG()
//         : boost::random_number_generator< rcss::random::DefaultRNG >(
//         DefaultRNG::instance() )
//       {}
// };
...
//#define drand(h,l)	(((((h)-(l)) * ((double)(random()%RANDOMBASE) /
//(double)RANDOMBASE))) + (l))
//#define	irand(x)	((random() / IRANDOMBASE) % (x))

inline int irand(int x) {
    if(x <= 1) return 0;
    // return boost::uniform_smallint<>( 0, x - 1 )(
    // rcss::random::DefaultRNG::instance() ); return (int)((double)x *
...
inline double drand(double low, double high) {
    ...
    // return boost::uniform_real<>( low, high )(
